...
public partial class benh_do_rirus : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (((DataView)odsBenhVirus.Select()).Count <= DataPager1.PageSize)
            {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["vt"]))
            {
                ...
                var dv = oProduct.ProductSelectOne(Request.QueryString["vt"]).DefaultView;
            ...
            {
                strTitle = strMetaTitle = "Bệnh do virus";
                strDescription = "";
                strMetaDescription = "";
            }
