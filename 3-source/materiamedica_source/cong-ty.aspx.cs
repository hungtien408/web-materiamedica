...
public partial class cong_ty : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (((DataView)odsAbout.Select()).Count <= DataPager1.PageSize)
            {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["ct"]))
            {
                ...
                var dv = oProduct.ProductSelectOne(Request.QueryString["ct"]).DefaultView;
